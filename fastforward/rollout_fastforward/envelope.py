"""Signed observation envelopes minted by Fast-Forward.

Byte-compatible with mcp-servers/gcp/envelope.py BY DESIGN: rollout-intel
verifies every piece of evidence with one shared verifier, so this module
must use the same _SIGNED_FIELDS, the same canonical JSON encoding, and the
same HMAC-SHA256 key from OBS_SIGNING_KEY. A fastforward_result envelope is
indistinguishable in shape from a gcp-observe one; any drift here silently
downgrades temporal evidence to insufficient_evidence at policy time.
"""
...
def mint(obs_type: str, scope: dict, payload, quality: dict | None = None,
         source: str = "rollout-fastforward", ttl_seconds: int = 600) -> dict:
    now = time.time()
    envelope = {
