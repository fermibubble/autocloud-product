...
| **`defer_verification`** (lifecycle: schedule re-check, stop tools, end turn) | Yes — it becomes the clock | Your defer/scheduler mechanism drives the checkpoint ladder — agent-armed defer is a FIRST-CLASS conformant shape (AGENT-CONTRACT §4 variant B), not a transition step | The reviewer arms it exactly once, AFTER `record_checkpoint` as its last action, with the record response's `next_check.delay_seconds` and the trigger's `unique_id` — the recorder's post-clamp decision, never the agent's own number — and arms nothing when `next_check_at` is null (ladder ended). Wiring: `gcp-harness/SESSION-LIFECYCLE.md` §3; agent rule: skill `references/clock.md` "Arming the next check". A dead session is re-fired harmlessly (replay-safe recording, idempotent `begin_review`) |
