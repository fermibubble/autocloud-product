...
Design:
  ...
  - DDL runs OUT OF BAND: the committed bq/*.schema.json files (the
    ...
    deploy time. export_episode therefore defaults to
    ensure_schema=False; runtime creation remains available as a
    dev/sandbox bootstrap.
"""
...
def _bq_type(col: str) -> str:
    # Legacy type names (INTEGER/FLOAT) deliberately: they are what
    # `bq mk --schema` files conventionally use AND what the API echoes
    # back for live tables, so schema_drift compares like with like.
...
# Standard-SQL aliases the API may hand back, normalized to the legacy
# names this module uses.
...
def schema_drift(live_fields: list[tuple[str, str]],
    ...
    same-name different-type column is a conflict. Type aliases
    (INT64/INTEGER etc.) are normalized before comparing."""
    def norm(t: str) -> str:
        t = str(t).upper()
        return _TYPE_ALIASES.get(t, t)

    wanted = {f["name"]: f for f in bq_schema(table)}
    live = {name: norm(bq_type) for name, bq_type in live_fields}
    missing = [dict(f, mode="NULLABLE") for name, f in wanted.items()
               if name not in live]
    conflicts = [f"{name}: live {live[name]} vs expected {norm(f['type'])}"
                 for name, f in wanted.items()
                 if name in live and live[name] != norm(f["type"])]
...
def ensure_dataset_and_tables(client, project: str,
    ...
    """Runtime bootstrap (dev/sandbox): idempotently create the
    dedicated, labeled dataset and its tables. The PRODUCTION flow is
    out-of-band DDL from the committed bq/*.schema.json files
    (bq/setup-bq.sh) with export_episode's default ensure_schema=False.
    A pre-existing dataset NOT carrying this module's ownership label
    is refused - never written into, never altered. A live table
...
def export_episode(db_path: str, *, phase: str,
    ...
    The dataset and tables are expected to exist already — created out
    of band from the committed bq/*.schema.json files (bq/setup-bq.sh).
    ensure_schema=True opts into runtime bootstrap creation instead
