...
Design:
  ...
  - APPEND-ONLY snapshots: every exported row carries exported_at
    ...
    analysts read through the *_latest views (bq/setup-views.sh -
    newest snapshot per primary key, outcome_final outranking
    ladder_complete on timestamp ties). Content-hashed row ids let BigQuery's
  ...
  - DDL lives ELSEWHERE, entirely: bq/generate_ddl.py produces the
    committed bq/*.schema.json files and the view SQL from this
    module's data-model maps (a parity test pins them together);
    bq/setup-bq.sh creates the dataset+tables, bq/setup-views.sh the
    *_latest views. A missing table at export time is a setup error
    and says so ("run bq/setup-bq.sh") rather than being retried or
    created on the fly.
"""
...
# _INT_COLS / _FLOAT_COLS / _EXPORT_COLS are part of the data-model
# surface consumed by bq/generate_ddl.py (schema-file generation);
# only _TIMESTAMP_COLS is also used by the streaming path itself.
