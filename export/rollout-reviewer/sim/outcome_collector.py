...
# Grading horizons — override with OUTCOME_HORIZONS="30m,2h,24h" to
# match the policy pack's `outcome_horizons` (units: m/h/d). The last
# horizon is the final one.
def _parse_horizons(spec: str) -> list[tuple[str, int]]:
    unit_minutes = {"m": 1, "h": 60, "d": 1440}
    out = []
    for token in [t.strip() for t in spec.split(",") if t.strip()]:
        unit = token[-1].lower()
        if unit not in unit_minutes or not token[:-1].isdigit():
            raise SystemExit(f"bad horizon {token!r} (use e.g. 30m, 2h, 7d)")
        out.append((token, int(token[:-1]) * unit_minutes[unit]))
    if not out:
        raise SystemExit("OUTCOME_HORIZONS parsed to an empty list")
    return out


HORIZONS = _parse_horizons(os.environ.get("OUTCOME_HORIZONS", "30m,2h,24h"))
